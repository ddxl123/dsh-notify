 ...
 *
 * @module dsh-notify/lib/core/text
 */
...
export function renderTextBody(input) {
  ...
  lines.push('')
  lines.push('Sent by dsh-notify (DeepSeek Harness).')
  return `${lines.join('\n').trimEnd()}\n`
}
...
export function renderHtmlBody(input) {
  ...
  return [
    ...
    '</table>',
    '</div>',
    `<div style="padding:12px 22px;background:#fafbfc;border-top:1px solid #eceff1;font-size:11px;color:#80868b">Sent by dsh-notify · ${escapeHtml(input.at === undefined ? '' : new Date(input.at).toISOString())}</div>`,
    '</div></body></html>',
