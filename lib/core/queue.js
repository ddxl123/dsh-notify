 ...
 *
 * @module dsh-notify/lib/core/queue
 */
...
export class Outbox {
  ...
  load() {
    ...
    try {
      ...
      this.onError(`dsh-notify: outbox at ${this.path} is not valid JSON and was reset (${describeError(error)})`)
      this.items = []
      return { loaded: 0, dropped: 0 }
    }
  ...
  save() {
    try {
      ...
      this.onError(`dsh-notify: could not persist the outbox at ${this.path} (${describeError(error)})`)
    }
  }
